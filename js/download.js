(function() {
	...
	function mainCtrl($http,$scope) {
		...
		function getTextData(id) {
			$http.get('http://localhost:5323/platform/data/examData!getGroupJSON',{params:{'groupId':id}}).then(function(res) {
				that.textData = res.data;
			});
		}
		...
		$scope.$on('imgData4',function(e,res) {
			that.imgData4 = res.imgData;
		});

		$scope.$on('imgData6',function(e,res) {
			that.imgData6 = res.imgData;
		});
					            ...
					            onrendered: function (canvasc) {
					            	...
					            	html2canvas($('#educationBar'), {
							            onrendered: function (canvase) {
							            	imageData4 = that.imgData4;
							            	imageData5 = canvase.toDataURL("png");
							            	imageData6 = that.imgData6;
							            	console.log(imageData4);return;
							            	var formStr = '<form method="post" action="http://localhost:5323/platform/data/examData!downloadPdf?groupId='+id+'">';
	...
	function gardeSectionDirective() {
		return {
			...
			link:function(scope,element,attr) {
				var myChart = myChart = echarts.init(element[0]);
	...
	function collegeDirective() {
		return {
			...
			replace:true,
			scope:{'data':'='},
			template:'<div style="width:900px;height:600px;margin:20px auto;"></div>',
			link:function(scope,element,attr) {
				...
				scope.$watch('data',function(res) {
					...
					scope.$emit('imgData6',{imgData:imgData});
				});
			}
		}
	}
		...
		return {
			...
			link:function(scope,element,attr) {
				...
				scope.$watch('data',function(res) {
					...
					myChart.setOption(option);
					var imgData = myChart.getDataURL();
					scope.$emit('imgData4',{imgData:imgData});
				});	
	...
	function scoreRatesDirective() {
		return {
			...
			replace:true,
			scope:{'data':'='},
			template:'<div style="width:100%;height:100%;padding:0 0 0 0;"><div class="item" ng-show="typeList[0]"><div class="chart"></div><div class="name">单选题<br />排名：{{typeList[0].OVER}}/{{typeList[0].ALLCOUNT}}</div></div><div class="item" ng-show="typeList[1]"><div class="chart"></div><div class="name">多选题<br />排名：{{typeList[1].OVER}}/{{typeList[1].ALLCOUNT}}</div></div><div class="item" ng-show="typeList[2]"><div class="chart"></div><div class="name">判断题<br />排名：{{typeList[2].OVER}}/{{typeList[2].ALLCOUNT}}</div></div><div class="item" ng-show="typeList[3]"><div class="chart"></div><div class="name">填空题<br />排名：{{typeList[3].OVER}}/{{typeList[3].ALLCOUNT}}</div></div><div class="item" ng-show="typeList[4]"><div class="chart"></div><div class="name">简答题<br />排名：{{typeList[4].OVER}}/{{typeList[4].ALLCOUNT}}</div></div><div class="item" ng-show="typeList[5]"><div class="chart"></div><div class="name">分析题<br />排名：{{typeList[5].OVER}}/{{typeList[5].ALLCOUNT}}</div></div></div>',
			link:function(scope,element,attr) {
